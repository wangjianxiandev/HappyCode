...
public class JavaScriptDirectInterface {
     ...
     */
    JavaScriptDirectInterface(ScratchJrActivity scratchJrActivity/*, MyApplication application*/) {
     ...
     */
    @JavascriptInterface
    public void showRating(final int count) {
        ...
        linearLayout = new LinearLayout(_activity);
        linearLayout.setOrientation(LinearLayout.VERTICAL);
        linearLayout.setHorizontalGravity(1);
        RatingBar ratingBar = new RatingBar(_activity);
        ...
        button.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                SharedPreferences sharedPreferences = _activity.getSharedPreferences("AllLevel", MODE_PRIVATE);
                int clickedLevel = sharedPreferences.getInt("clickedLevel", 1);
                clickedLevel--;
                //用户在此类游戏已经解锁的最大关卡数
                int gameUnlockLevel = sharedPreferences.getInt("scratchGameGuideBlockUnlockLevel", 0);
                //当前关卡的星星=0，说明用户第一次进入该游戏关卡，更新星星的数据
                if(resultCount>=2){
                    if ((clickedLevel >= gameUnlockLevel - 1)) {
                        SharedPreferences.Editor editor = _activity.getSharedPreferences("AllLevel", MODE_PRIVATE).edit();
                        editor.putInt("scratchGameGuideBlockUnlockLevel", clickedLevel + 2);
                        Toast.makeText(_activity,"un "+String.valueOf(gameUnlockLevel),Toast.LENGTH_SHORT).show();
                        editor.commit();
                    }
                    List<LevelInfo> rates = DataSupport.where("name = ?", "scratchGameGuideBlock " + (clickedLevel-1)).find(LevelInfo.class);
                    LevelInfo rate ;
                    if(rates.isEmpty()||resultCount>rates.get(0).getRating()) {
                        if (rates.isEmpty()){
                            rate = new LevelInfo();
                        }else  {
                            rate = rates.get(0);
                        }
                        rate.setName("scratchGameGuideBlock " + (clickedLevel-1));
                        rate.setModel("scratchGameGuideBlock");
                        rate.setRating(resultCount);
                        rate.save();
                        Toast.makeText(_activity,"in"+String.valueOf(clickedLevel),Toast.LENGTH_SHORT).show();
                    }
                        //只有存在的最大关卡大于已解锁的最大关卡， 数据库中的“最大关卡”才会+1
                    Toast.makeText(_activity,"out ",Toast.LENGTH_SHORT).show();
                }
                ad.dismiss();
            }
...
//    @JavascriptInterface
//    public void makeAToast(int count){
//        Toast.makeText(_activity, String.valueOf(count), Toast.LENGTH_SHORT).show();
//    }
